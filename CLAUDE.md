...
- **`src/ui/`** — React components (dialogs, toolbar, palette, status bar, usage panel).
...
- **`src/render/previewModels.ts`** — the per-model shapes (pocket door, hero, world tree, tree spirit, wolf, golem, carnivorous plant, demon statue), all built from boxes/spheres/cones/icosahedra. Materials are shared across the whole scene and disposed once; geometries are per-model and disposed with the map. Self-lit parts (the world tree's light-fruit, the spawn ring, the tree spirit's/wolf's/golem's eyes, the golem's core) are tagged `userData.glow` so `prepareShadows` keeps them out of shadow casting.
...
**Per-brush 3D model types.** A brush's `previewModel` (`PREVIEW_MODELS` in `core/brush.ts`) decides what its cells become in the 3D preview: `default` / `special-wall` (11.1 tall) / `special-door` (same height; a bi-parting pocket door whose leaves slide into slots between the front and back skins of the flanking wall — it reuses the special-wall *material instance*, so its color can't drift from the wall's) / `outer-wall` (22.5 tall) / `start-area` (a white bob-haired female hero with a sword, on a glowing spawn ring, facing away from the default camera) / `safe-area` (a world tree far larger than one cell) / `boss-area` (a giant tree spirit) / `monster` (a black wolf) / `golem` (a mossy stone golem with a glowing core) / `plant` (a carnivorous plant) / `statue` (a horned, winged demon statue on a gold-trimmed pedestal). **The keys are frozen; only the Korean display names in `PREVIEW_MODEL_NAME` ever change**, because brush files store the key — renaming `boss-area` would silently reset every brush using it through the schema's `.catch('default')`. That's why keys and names disagree: `start-area` shows as 용사, `safe-area` as 세계수, `boss-area` as 나무 정령, `monster` as 늑대. `forEachPreviewCell` resolves this per cell and splits it into two callbacks:

- Wall-family models only apply where the *preview* terrain is `TERRAIN_WALL`. If the generator carved that cell open, nothing is built there regardless of what the brush says — the preview grid wins over the brush setting. **`special-door` is the one exception**: a door is a gap in a wall rather than a wall, so a carved-open door cell still gets its frame, drawn with the leaves retracted into their pockets, while a door cell that stayed `TERRAIN_WALL` is drawn shut. Because a door is a group of parts rather than one instanced box, `forEachPreviewCell` reports it through its own `door(open, height, …)` visitor instead of `wall`.
...
The one thing that animates without a store update is the **brush flash** (clicking a row in the usage panel makes that brush's cells blink). The store holds only `flash: { brushId, startedAt }`, set and cleared twice per click; the rAF loop forces a redraw for as long as it is set and converts elapsed time into this frame's alpha via `brushFlashAlpha()`, so the per-frame value never round-trips through React. Keep new time-varying overlays on that pattern rather than putting an animation frame counter in the store.

### Usage panel

`src/ui/UsagePanel.tsx` is the collapsible panel overlaid on the canvas's bottom-left: which brushes the map currently uses and how many cells each covers, with a click flashing those cells. The count comes from `countBrushUsage()` (`core/tools/usage.ts`), which sums **both** layers' `brush` grids by brush id — a brush's `layer` can be edited after painting, so its id may sit in either grid — and deliberately keeps ids whose brush was deleted from the palette (the panel labels them `알 수 없음(#id)`), since those cells are still in the map.

Scanning the whole map costs ~8ms on the largest allowed map, so the panel recomputes only while expanded and only on **`docRev`**, not `rev` (see below).
...
- `editorStore.ts` — the current `MapDoc`, camera, tool selection, undo/redo, autosave scheduling, and the maze preview buffer. Two counters look alike and are not: `rev` means "the canvas should redraw" and is bumped by hover and camera moves as well as edits, while `docRev` counts only changes to the map's *contents* (bumped in `afterMutation`/`replaceDoc`). Anything that rescans the whole map must key off `docRev`, or it re-runs on every mouse move. Note also that in-progress strokes mutate the typed arrays in place and bump neither counter until `endStroke`. History and in-progress stroke state (`activeStroke`, `strokeTool`, `strokeBlockReason`) are deliberately kept in module-level variables outside Zustand state, since they mutate many times per second during a drag and don't need to trigger React re-renders.
